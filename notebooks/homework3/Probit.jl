...
# ╔═╡ 09d3aacf-bf2b-4216-a84d-9a1a662431c8
using PlutoUI
...
# ╔═╡ a8a36d4b-8aee-48f6-a7b3-b97439474966
makeData = function(n::Integer, β = [1; 1.5; -0.5], k = 3)
	normal = Normal(0,1)
	ϵ = rand(normal, n)
	Σ = Matrix(1.0I, 3, 3)
	mv_normal_dist = MvNormal(Σ)
	x = rand(mv_normal_dist, n)
	y = x' * β + ϵ
	y_star = [ifelse(y[i] >= 0, 1, 0) for i in 1:n]
	return Dict(:β => β, :n => n, :X => x', :y => y_star, :dist => normal)
end
...
# ╔═╡ 27fda577-8ba2-4bff-85c8-27bf241da20b
loglik = function(betas::Vector, d::Dict)
	Xβ = d[:X] * betas
	ϕ = cdf.(d[:dist], Xβ)
	L = [ifelse(d[:y][i] == 1, log(ϕ[i]), log(1-ϕ[i])) for i in 1:d[:n]]
	return -1/d[:n] * sum(L)
end
...
# ╔═╡ ea5c2be1-1df3-40ae-b6b9-40d891a3586b
md"""
n = $(@bind n Slider(0:100:10000, show_value = true, default = 100))
"""
...
# ╔═╡ 33626b15-10d8-4b0d-bb00-dba54d1d72a4
maximize_like = function(; x0 = X0)
	d = makeData(10000)
    res = optimize(x -> loglik(x, d), x0, NelderMead())
    return res
end

# ╔═╡ b79688a4-a9b2-4d58-8fbe-f09b42f8626d
md""" Quite close to β! """

# ╔═╡ 463fecc3-9084-4933-b17b-8357b92f8a28
begin
	res = maximize_like(; x0 = X0)
	Optim.minimizer(res)
end
...
# ╔═╡ b3b68d98-d5f2-4298-8f93-2f8bffb1479c
function grad1!(g::Vector, betas::Vector, d::Dict)
    g .= ForwardDiff.gradient(b -> loglik(b, d), betas)
    return nothing
end

# ╔═╡ 26824986-a16e-4ed0-8330-d150c47112ca
function grad!(g::Vector, betas::Vector, d::Dict)
    Xβ = d[:X] * betas
    Φ = cdf.(d[:dist], Xβ)
    ϕ = pdf.(d[:dist], Xβ)
    ...
    for j in 1:length(betas)
        ...
        for i in 1:d[:n]
            if d[:y][i] == 1
                sum_term += ϕ[i] * (1 / Φ[i]) * d[:X][i, j]
            else
                sum_term -= ϕ[i] * (Φ[i] / (1 - Φ[i])) * d[:X][i, j]
            end
        end
        g[j] = -1/d[:n] * sum_term
    end
...
# ╔═╡ 110c3204-e85e-44f7-bd25-35ec5cb837f2
function maximize_like_grad(; x0 = X0)
	d = makeData(10000)
    res = optimize(x -> loglik(x, d), g -> grad!(g, x, d), x0, NelderMead())
	...
	b = d[:β]
	println("Optimized values: $m")
	println("True values: $b")
    return res
end
...
# ╔═╡ 2679b1c7-7012-46a6-986c-23d003861f2f
function hessian1!(h::Matrix, betas::Vector, d::Dict)
	h .= Calculus.hessian(x -> loglik(x,d), betas)
	return nothing
end

# ╔═╡ f5651c2e-785e-48fb-8309-8902921f1c23
function hessian2!(h::Matrix, betas::Vector, d::Dict)
    h .= ForwardDiff.hessian(b -> loglik(b, d), betas)
    return nothing
end

# ╔═╡ 265e0ed6-2cec-4400-adcc-39d763c5c936
function hessian!(h::Matrix, betas::Vector, d::Dict)
    n = d[:n]
    X = d[:X]
    y = d[:y]
    dist = d[:dist]
    ...
    for i in 1:length(betas)
        for j in 1:length(betas)
            ...
            for k in 1:n
                ...
                if y[k] == 1
                    sum_term += (s_i * ϕ[k] * (1 / Φ[k]) + ϕ[k]^2 / Φ[k]^2) * x_ik * x_jk
                else
                    sum_term += (ϕ[k]^2 / ((1 - Φ[k])^2) - s_i * ϕ[k] / (1 - Φ[k])) * x_ik * x_jk
                end
            end
            h[i, j] = 1/n * sum_term
        end
...
# ╔═╡ 02df1bbb-7ef1-44c4-b8d6-dfd811f5764d
function maximize_like_grad_hess(; x0 = X0)
	d = makeData(10000)
    res = optimize(x -> loglik(x, d), g -> grad!(g, x, d), h -> hessian!(h, x, d), x0, NelderMead())
	 if Optim.converged(res)
        println("Optimization converged successfully.")
    else
        println("Optimization did not converge.")
...
function inv_observedInfo(betas::Vector, d)
    ...
    hessian!(h, betas, d)
    io_hessian = inv(h) / d[:n]
    return io_hessian
end

# ╔═╡ 47576313-d768-4c2a-90f6-6920ffa56a92
md"I modified number of observations to 100,000. Sometimes it says the se are correct, sometimes not."

# ╔═╡ 64995e54-75f6-11eb-39c7-61406ddc7a6e
md"Define finally the function `inv_Info(betas::Vector,d)` here!"

# ╔═╡ 3f1d7499-7c86-4951-ba03-225b46252458
function inv_Info1(betas::Vector, d::Dict)
    n = d[:n]
    X = d[:X]
    y = d[:y]
    dist = d[:dist]
    ...
    information = zeros(length(betas), length(betas))
    
    for i in 1:n
        s_i = Xβ[i]
        x_i = X[i, :]
        
        information += (ϕ[i]^2 / ((Φ[i] * (1 - Φ[i])))) * x_i * x_i'
    end
    
    return information^(-1)/d[:n]
end


# ╔═╡ b105d351-dda3-4989-b291-0fa79d56b580
...
# ╔═╡ 3a2ee8b0-1d2a-4d93-a96a-021364cdd5a2
let
	d = makeData(10000)
	betas = X0
	m = maximize_like_grad_hess()
	println(inv_observedInfo(betas, d))
	println(inv_Info(m.minimizer, d))
	println(inv_Info1(m.minimizer, d))
end
...
manifest_format = "2.0"
project_hash = "b6361696db3c112c7ae4bce0428451f7ab300ec4"
